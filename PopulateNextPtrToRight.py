...
class Solution:
    def connect(self, root: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while nodes:
            ...
            if nodes[0][0].left and nodes[0][0].right:
                queue.extend([(nodes[0][0].left, level + 1), (nodes[0][0].right, level + 1)])
                nodes.extend([(nodes[0][0].left, level + 1), (nodes[0][0].right, level + 1)])
